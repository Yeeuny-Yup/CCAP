...
- **Exact statistic used:** for each sample $x$, we compute channel-wise $|O_{F,C}(x)|_1$ by summing absolute activations over all non-channel dimensions. We then estimate $E[|O^{F,C}(x)|_1]$ over $D_{cal}$.
